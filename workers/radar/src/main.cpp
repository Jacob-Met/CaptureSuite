...
// Milestone 6 radar worker — Infineon FMCW raw frame capture (RADAR_PIPELINE.md).
 ...
 public:
  void push(capture::v1::MessageType type, const std::string& bytes) {
    ...
    if (type == capture::v1::MESSAGE_TYPE_PREVIEW_FRAME) {
      latest_preview_ = std::move(frame);
      return;
    }
...
int main(int argc, char** argv) {
  ...
  if (pipe_name.empty() || worker_id.empty()) {
    std::fprintf(stderr,
                 "usage: capture_worker_radar --pipe <name> --worker-id <id> "
                 "[--plugin radar.ifx]\n");
    return 2;
  }
  if (plugin_id.empty()) {
  ...
  if (pipe == INVALID_HANDLE_VALUE) {
    std::fprintf(stderr, "radar worker: failed to connect to %s\n",
                 pipe_name.c_str());
    return 1;
  }
  ...
  capture::radar_worker::WorkerSession session(plugin_id);
  ...
  for (;;) {
    ...
    if (type == capture::v1::MESSAGE_TYPE_IDENTIFY) {
      ...
      *reply.mutable_manifest() = session.build_manifest();
      ...
      std::fprintf(stderr, "radar worker: ignoring message_type=%u\n",
                   frame.message_type);
    }
  }

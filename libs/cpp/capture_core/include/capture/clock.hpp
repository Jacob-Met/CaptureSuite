...
struct WallAnchor {
  // FILETIME-style 100ns intervals since 1601 UTC, for provenance only.
...
// Monotonic session clock based on QueryPerformanceCounter.
// Session time is never derived from wall clock.
class SessionClock {
 public:
  SessionClock();
  ...
  // Nanoseconds since an arbitrary process start (monotonic).
  int64_t now_monotonic_ns() const;

  bool has_t0() const { return t0_.has_value(); }
  const T0& t0() const { return *t0_; }

  // Establish T0 with an interleaved QPC/wall triple. Returns uncertainty_ns.
  T0 establish_t0();
